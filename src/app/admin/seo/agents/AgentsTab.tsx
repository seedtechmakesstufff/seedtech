    ...
    <div className="space-y-8">
      ...
      {showRunAllConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowRunAllConfirm(false)} />
          <div className="relative bg-dark-elevated border border-white/[0.08] rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-seed-500/10 mx-auto mb-4">
              <Play className="w-5 h-5 text-seed-400" />
            </div>
            <h3 className="text-base font-semibold text-white text-center">Run all weekly agents?</h3>
            <p className="text-sm text-white/50 text-center mt-2">
              This calls Claude multiple times and takes 2–5 minutes. Results will appear below when complete.
            </p>
            <div className="flex gap-3 mt-6">
              <button
                onClick={() => setShowRunAllConfirm(false)}
                className="flex-1 text-sm font-semibold px-4 py-2.5 rounded-lg border border-white/10 hover:border-white/20 text-white/70 hover:text-white transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={runAll}
                className="flex-1 text-sm font-semibold px-4 py-2.5 rounded-lg bg-seed-500 hover:bg-seed-400 text-dark-base transition-colors flex items-center justify-center gap-2"
              >
