...
function PingPongLottie({ animationData, className }: { animationData: object; className?: string }) {
  ...
  return (
    <Lottie
      ...
      autoplay
      className={className}
      onDOMLoaded={() => { mounted.current = true; }}
      onComplete={() => {
...
export const autopilotGlassStyle: React.CSSProperties = {
  borderRadius: 16,
...
export const pillars: PillarData[] = [
  ...
  },
  {
    ...
    miniTitle: "ALL YOUR SEO SIGNALS IN ONE COMMAND CENTER",
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 z-10 w-[160px] h-[160px] pointer-events-none select-none">
        {pillar.lottie ? (
          <PingPongLottie animationData={pillar.lottie} className="w-full h-full drop-shadow-2xl" />
        ) : (
          <Image
            src={pillar.image}
      ...
      <motion.div
        ...
        style={{
          ...autopilotGlassStyle,
          borderColor: isActive ? "rgba(74, 222, 128, 0.30)" : "rgba(255, 255, 255, 0.12)",
        }}
      >
        {isActive && (
          <motion.div
...
// ─── AutopilotCards — the reusable interactive mini+detail block ───────────────

export function AutopilotCards() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <motion.div
        ...
        className="relative"
        style={{ ...autopilotGlassStyle, borderRadius: 24, overflow: "visible" }}
      >
        <div className="relative flex flex-col lg:flex-row items-center lg:items-stretch min-h-[300px]">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="inline-flex items-baseline gap-2 self-start">
                  <span className="font-display text-3xl font-bold text-seed-400">{active.stat}</span>
                  <span className="text-xs text-white/40 uppercase tracking-wider">{active.statLabel}</span>
                </div>
          ...
          <div className="relative w-full lg:w-[360px] shrink-0 flex items-end justify-center lg:justify-end pointer-events-none select-none">
            <AnimatePresence mode="wait">
              ...
              >
                {active.lottie ? (
                  <PingPongLottie animationData={active.lottie} className="absolute inset-0 w-full h-full" />
                ) : (
                  <Image
                    src={active.image}
